  ...
  def index = SecuredAction.async { implicit request =>
    ...
    for {
      ...
      yield {
        ...
        Ok(views.html.requests.index(request.identity, SortingCenterStockForm.form, supplies, resourceCategoryOptions, resourceAmountLabelOptions))
      }
  }
      ...
      form => {
        ...
        for {allSuppliesExceptUser <- fAllSuppliesExceptUser; resourceCategories <- fResourceCategories; resourceAmountLabels <- fResourceAmountLabels}
          yield {
            ...
            BadRequest(views.html.requests.index(request.identity, form, supplies, resourceCategoryOptions, resourceAmountLabelOptions))
          }
      },
      data => {
